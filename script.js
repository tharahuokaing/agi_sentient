/**
 * script.js
 * Comprehensive interactivity file for AGI vs Sentience processing parameters.
 */

const PARADIGM_MESSAGES = {
...
const ParadigmAnalyzer = {
    /**
     * Triggers the UI and alert feedback based on the system type.
     * @param {'agi' | 'sentient'} type 
     */
    runAnalysis(type) {
        if (!PARADIGM_MESSAGES[type]) return;

        // Visual feedback for Sentient type
        if (type === 'sentient') {
            this._triggerShakeEffect('sentient-box');
        }

        alert(PARADIGM_MESSAGES[type]);
    },

    /**
     * Private method to handle CSS class toggling
     * @param {string} elementId 
     */
    _triggerShakeEffect(elementId) {
        const targetBox = document.getElementById(elementId);
        if (!targetBox) return;

        targetBox.classList.add('animate-shake');
        setTimeout(() => targetBox.classList.remove('animate-shake'), 400);
    }
};
